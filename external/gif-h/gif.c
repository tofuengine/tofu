...
static void GifMakePalette( const void* lastFrame, const void* nextFrame, uint32_t width, uint32_t height, int bitDepth, bool buildForDither, GifPalette* pPal )
{
...
static void GifWritePalette( const GifPalette* pPal, FILE* f )
{
    ...
    for(int ii=1; ii<(1 << pPal->bitDepth); ++ii)
    {
        ...
        // Invert B and R components in the case the image is packed in BGRA8 format (like on Windows for OpenGL)
...
bool GifIsWriting(const GifWriter* writer)
{
    return writer->f;
}
