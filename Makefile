...
CFLAGS=-D_DEFAULT_SOURCE -DLUA_32BITS -DLUA_FLOORN2I=F2Ifloor -DSTBI_ONLY_PNG -DSTBI_NO_STDIO -DDR_FLAC_NO_STDIO -DMA_NO_DECODING -DMA_NO_ENCODING -DMA_NO_GENERATION -DLIBXMP_CORE_PLAYER -Isrc -Iexternal
...
SOURCES+=$(wildcard external/glad/*.c external/GLFW/*.c external/lua/*.c external/miniaudio/*.c external/spleen/*.c external/xm/*.c external/xmp-lite/*.c)
...
INCLUDES+=$(wildcard external/glad/*.h external/GLFW/*.h external/lua/*.h external/miniaudio/*.h external/dr_libs/*.h external/spleen/*.h external/stb/*.h external/xm/*.h  external/xmp-lite/*.h)
OBJECTS:=$(SOURCES:%.c=%.o)
