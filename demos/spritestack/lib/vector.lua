...
Copyright (c) 2019 by Marco Lizza (marco.lizza@gmail.com)
...
function Vector2D.from_polar(a, l, ox, oy)
  return Vector2D.new(math.cos(a) * l + (ox and ox or 0), math.sin(a) * l + (oy and oy or 0))
end

function Vector2D:to_polar()
...
function Vector2D:is_zero()
  return self.x == 0 and self.y == 0
end

function Vector2D:is_equal(v)
  return self.x == v.x and self.y == v.y
end

function Vector2D:assign(v)
  self.x = v.x
  self.y = v.y
  return self
end
...
function Vector2D:perp_dot(v)
  return (self.x * v.y) - (self.y * v.x)
end

function Vector2D:magnitude_squared()
...
function Vector2D:distance_to_squared(v)
  local dx = self.x - v.x
  local dy = self.y - v.y
  return (dx * dx) + (dy * dy)
end

function Vector2D:distance_to(v)
  return math.sqrt(self:distance_to_squared(v))
end

function Vector2D:normalize(l)
  return self:scale((l or 1) / self:magnitude())
end

function Vector2D:normalize_if_not_zero(l)
...
function Vector2D:trim_if_not_zero(l)
  if self:is_zero() then
    return self
  end
  return self:trim(l)
...
function Vector2D:angle_between(v)
